 ...
 */
k4a_result_t logger_register_callback(k4a_logging_cb_t *message_cb,
...
TraceError(k4a_result_t result, const char *szCall, const char *szFile, int line, const char *szFunction)
{
    if (K4A_FAILED(result))
    {
        ...
        logger_log(K4A_LOG_LEVEL_ERROR, "%s (%d): %s returned failure in %s()", szFile, line, szCall, szFunction);
    }
    return result;
}
...
FORCEINLINE k4a_result_t TraceReturn(k4a_result_t result, const char *szFile, int line, const char *szFunction)
{
    if (K4A_FAILED(result))
    {
        ...
        logger_log(K4A_LOG_LEVEL_ERROR, "%s (%d): %s() returned failure.", szFile, line, szFunction);
    }
    else
    {
        logger_log(K4A_LOG_LEVEL_TRACE, "%s (%d): %s() returned success.", szFile, line, szFunction);
    }
    return result;
}
...
#define LOG_TRACE(message, ...)                                                                                        \
    logger_log(K4A_LOG_LEVEL_TRACE, "%s (%d): %s(). " message, __FILE__, __LINE__, __func__, __VA_ARGS__)
#define LOG_INFO(message, ...)                                                                                         \
    logger_log(K4A_LOG_LEVEL_INFO, "%s (%d): %s(). " message, __FILE__, __LINE__, __func__, __VA_ARGS__)
#define LOG_WARNING(message, ...)                                                                                      \
    logger_log(K4A_LOG_LEVEL_WARNING, "%s (%d): %s(). " message, __FILE__, __LINE__, __func__, __VA_ARGS__)
#define LOG_ERROR(message, ...)                                                                                        \
    logger_log(K4A_LOG_LEVEL_ERROR, "%s (%d): %s(). " message, __FILE__, __LINE__, __func__, __VA_ARGS__)
