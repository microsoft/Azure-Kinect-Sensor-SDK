 ...
 *
 * \param context
 * The context of the callback function. This is the context that was supplied by the caller as \p
 * k4a_set_message_handler.
 *
 * \param level
 ...
 *
 * \remarks
 * The callback is called asynchronously when the Azure Kinext SDK generates a message at a \p level that is equal
 * or more critical to the level specified when calling \r k4a_set_message_handler an registering the callback.
 *
 * \remarks
 * The user must ensure synchronization between parallel calls to the k4a_logging_cb_t function.
 *
 * \xmlonly
 * <requirements>
 *   <requirement name="Header">k4atypes.h (include k4a/k4a.h)</requirement>
