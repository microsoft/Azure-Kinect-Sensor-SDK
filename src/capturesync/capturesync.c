...
k4a_result_t capturesync_start(capturesync_t capturesync_handle, const k4a_device_configuration_t *config)
{
    ...
    sync->fps_period = HZ_TO_PERIOD_US(k4a_convert_fps_to_uint(config->fps_mode_info.mode_id));
    ...
    if (config->color_mode_info.mode_id == K4A_COLOR_RESOLUTION_OFF || config->depth_mode_info.mode_id == K4A_DEPTH_MODE_OFF)
    {
