...
k4a_result_t parse_recording_config(k4a_playback_context_t *context)
{
    ...
    if (context->color_track)
    {
        ...
        context->record_config.color_mode_info.mode_id = K4A_COLOR_RESOLUTION_OFF;
        for (size_t i = 0; i < arraysize(color_resolutions); i++)
        {
            ...
            if (k4a_convert_resolution_to_width_height(color_resolutions[i], &width, &height))
            {
                ...
                {
                    context->record_config.color_mode_info.mode_id = color_resolutions[i];
                    break;
                }
            }
        }

        if (context->record_config.color_mode_info.mode_id == K4A_COLOR_RESOLUTION_OFF)
        {
    ...
    context->record_config.depth_mode_info.mode_id = K4A_DEPTH_MODE_OFF;
    ...
    if (depth_mode_tag != NULL)
    {
        ...
        for (size_t i = 0; i < arraysize(depth_modes); i++)
        {
            if (depth_mode_str == depth_modes[i].second)
            {
                ...
                {
                    context->record_config.depth_mode_info.mode_id = depth_modes[i].first;
                    break;
                }
            }
        }

        if (context->record_config.depth_mode_info.mode_id == K4A_DEPTH_MODE_OFF)
        {
            ...
            for (size_t i = 0; i < arraysize(legacy_depth_modes); i++)
            {
                if (depth_mode_str == legacy_depth_modes[i].second)
                {
                    if (k4a_convert_depth_mode_to_width_height(legacy_depth_modes[i].first,
                                                               ...
                                                               &depth_height))
                    {
                        context->record_config.depth_mode_info.mode_id = legacy_depth_modes[i].first;
                        break;
                    }
                }
            }
    ...
    if (frame_period_ns > 0)
    {
        ...
        {
        case 5:
            context->record_config.fps_mode_info.mode_id = K4A_FRAMES_PER_SECOND_5;
            break;
        case 15:
            context->record_config.fps_mode_info.mode_id = K4A_FRAMES_PER_SECOND_15;
            break;
        case 30:
            context->record_config.fps_mode_info.mode_id = K4A_FRAMES_PER_SECOND_30;
            break;
        default:
