...
k4a_result_t depth_start(depth_t depth_handle, const k4a_device_configuration_t *config)
{
    ...
    if (K4A_SUCCEEDED(result))
    {
        ...
        result = TRACE_CALL(depthmcu_depth_set_capture_mode(depth->depthmcu, config->depth_mode_info));
    }
    ...
    if (K4A_SUCCEEDED(result))
    {
        result = TRACE_CALL(depthmcu_depth_set_fps(depth->depthmcu, config->fps_mode_info));
    }
