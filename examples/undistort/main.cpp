...
typedef enum
{
    ...
    INTERPOLATION_BILINEAR_INVALID_NEIGHBOR /**< Bilinear interpolation with invalidation when neighbor contain invalid
                                               data with value 0 */
} interpolation_t;
                                    ...
                                    interpolation_t type)
{
    ...
    for (int y = 0, idx = 0; y < pinhole->height; y++)
    {
        ...
        for (int x = 0; x < pinhole->width; x++, idx++)
        {
            ...
            else if (type == INTERPOLATION_BILINEAR || type == INTERPOLATION_BILINEAR_INVALID_NEIGHBOR)
            {
            ...
            if (valid && src.x >= 0 && src.x < src_width && src.y >= 0 && src.y < src_height)
            {
                ...
                if (type == INTERPOLATION_BILINEAR || type == INTERPOLATION_BILINEAR_INVALID_NEIGHBOR)
                {
    ...
    for (int i = 0; i < dst_width * dst_height; i++)
    {
        if (lut_data[i].x != INVALID && lut_data[i].y != INVALID)
        {
            ...
            else if (type == INTERPOLATION_BILINEAR || type == INTERPOLATION_BILINEAR_INVALID_NEIGHBOR)
            {
                ...
                // interpolation for current target pixel if one of the neighbors contains invalid data. If the image is
                // color or ir images, user should use INTERPOLATION_BILINEAR
                if (type == INTERPOLATION_BILINEAR_INVALID_NEIGHBOR)
                {
