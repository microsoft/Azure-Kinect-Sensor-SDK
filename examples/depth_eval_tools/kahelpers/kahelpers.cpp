...
namespace kahelpers
{
	...
	cv::Ptr<cv::aruco::CharucoBoard> charuco_target::create_board()
	{
		...
		cv::Ptr<cv::aruco::CharucoBoard> board = cv::aruco::CharucoBoard::create(
			squaresX, squaresY,
			squareLength_mm / 1000.0f, markerLength_mm / 1000.0f,
			dictionary);
		return board;
	}
	...
	k4a::image color_from_opencv(const cv::Mat &M)
	{
		...
		k4a::image img = k4a::image::create(K4A_IMAGE_FORMAT_COLOR_BGRA32,
			M4.cols, M4.rows, static_cast<int>(M4.step));
	...
	k4a::image depth_from_opencv(const cv::Mat &M)
	{
		...
		k4a::image img = k4a::image::create(K4A_IMAGE_FORMAT_DEPTH16,
	...
	k4a::image ir_from_opencv(const cv::Mat &M)
	{
		...
		k4a::image img = k4a::image::create(K4A_IMAGE_FORMAT_IR16,
			M.cols, M.rows, static_cast<int>(M.step));
	...
	bool write_calibration_blob(const std::vector<uchar> calibration_buffer, const std::string output_path, const std::string calib_name)
	{
	...
	void write_opencv_calib(const k4a_calibration_camera_t &camera_calibration,
		const std::string output_path, const std::string calib_name)
	{
	...
	void get_images(k4a::playback &playback, int timestamp,
		cv::Mat &ir16, cv::Mat &depth16, cv::Mat &color8,
		bool single, bool get_ir, bool get_depth, bool get_color)
	{
	...
	void find_common_markers(
		const std::vector<int> &id1, const std::vector<cv::Point2f> &corners1,
		const std::vector<int> &id2, const std::vector<cv::Point2f> &corners2,
		std::vector<int> &cid, std::vector<cv::Point2f> &ccorners1, std::vector<cv::Point2f> &ccorners2)
	{
		cid.clear();
		ccorners1.clear();
		ccorners2.clear();
	...
	void get_board_object_points_charuco(const cv::Ptr<cv::aruco::CharucoBoard> &board,
		const std::vector<int> &ids, std::vector<cv::Point3f> &corners3d)
	{
		corners3d.clear();
