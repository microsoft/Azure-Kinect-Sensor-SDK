	...
	for (int view_idx = 0; view_idx < num_views; view_idx++)
	{
		...
		for (int cap_idx = 0; cap_idx < num_caps_per_view; cap_idx++)
		{
			...
			if (dump_color)
			{
				...
				{
					std::cout << " | Color res: " << color_img.get_height_pixels() << "x" << color_img.get_width_pixels();
					cv::Mat frame32[3];
					cv::Mat bands[3];
			...
			if (dump_depth)
			{
				...
				{
					std::cout << " | Depth16 res: " << depth_img.get_height_pixels() << "x" << depth_img.get_width_pixels();
