...
int calculate_transformation_error(
	const cv::Mat &depth16, const cv::Mat &color8,
	const std::vector<cv::Point2f> &corners_d, const std::vector<cv::Point2f> &corners_c,
	const k4a::calibration &calibration,
	float &rms,
	cv::Mat &err_img, bool gen_err_img = true)
{
	rms = 0;
	int nValid = 0;
	...
	for (int i = 0; i < corners_d.size(); i++)
	{
		...
		bool valid = calibration.convert_2d_to_2d(pd, d_mm,
			K4A_CALIBRATION_TYPE_DEPTH,
			K4A_CALIBRATION_TYPE_COLOR,
			&pc);
		//3. if valid, calculate the reprojection error between:
		//		a) prediction: transformed markers from depth to color using intrinsics of both cameras and R&T between them
...
static bool process_mkv(
	const std::string &passive_ir_mkv,
	const std::string &depth_mkv,
	const std::string &template_file,
	int timestamp,
	const std::string &output_dir,
	float gray_gamma, float gray_max, float gray_percentile, bool save_images)
{
	...
	cv::Mat passive_ir;
	cv::Mat nullMat;
	get_images(playback_ir, timestamp,
		passive_ir, nullMat, nullMat, false, true, false, false);	
	...
	get_images(playback, timestamp,
		ir16, depth16, color8, false, true, true, true);
	...
	detect_charuco(passive_ir8,
		board, params,
		markerIds_ir, markerCorners_ir,
		charucoIds_ir, charucoCorners_ir,
		false);
	//
	...
	detect_charuco(color8,
		board, params,
		markerIds_color, markerCorners_color,
		charucoIds_color, charucoCorners_color,
		false);
	...
	find_common_markers(
		charucoIds_ir, charucoCorners_ir,
		charucoIds_color, charucoCorners_color,
		common_id, common_corners_ir, common_corners_color);
	...
	int n_valid = calculate_transformation_error(
		depth16, color8,
		common_corners_d, common_corners_color,
		calibration,
		rms,
		err_img, save_images);
...
int main(int argc, char **argv)
{
	...
	bool stat = process_mkv(
		...
		output_dir,
		gray_gamma,
		gray_max, gray_percentile, save_images);
