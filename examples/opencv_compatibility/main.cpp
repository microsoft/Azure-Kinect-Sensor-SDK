...
int main(int argc, char ** /*argv*/)
{
    ...
    if (depth_mode_count > 1)
    {
        for (uint32_t d = 1; d < depth_mode_count; d++)
        {
            ...
            {
                if (depth_mode.height <= 512 && depth_mode.vertical_fov <= 65)
                {
                    depth_mode_id = d;
                    break;
                }
    ...
    config.color_mode_id = color_mode_info.mode_id;  // K4A_COLOR_RESOLUTION_1080P;
    config.depth_mode_id = depth_mode_info.mode_id;       // K4A_DEPTH_MODE_WFOV_2X2BINNED;
    config.fps_mode_id = fps_mode_info.mode_id;      // K4A_FRAMES_PER_SECOND_30;
